...
import com.songbirdnest.facebook.FacebookAPI.DB;
...
public class Base64 {
        ...
        if (source == null) {
            ...
            while (i < off + len) {
                ...
                if (sbiDecode >= WHITE_SPACE_ENC) {
                    ...
                    throw new IOException(String.format("Bad Base64 input character decimal %d in array position %d", new Object[]{Integer.valueOf(source[i] & MotionEventCompat.ACTION_MASK), Integer.valueOf(i)}));
                }
            }
            b4Posn = b4Posn2;
    ...
    public static byte[] decode(String s, int options) throws IOException {
        ...
        if (bytes != null && bytes.length >= 4 && !dontGunzip && 35615 == ((bytes[0] & MotionEventCompat.ACTION_MASK) | ((bytes[1] << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK))) {
            ByteArrayInputStream bais = null;
            GZIPInputStream gzis = null;
            ByteArrayOutputStream baos = null;
            byte[] buffer = new byte[DB.sDBLikeStatusPendingUnlike];
            try {
                ByteArrayOutputStream baos2 = new ByteArrayOutputStream();
                try {
