...
public class VerticalSeekBar extends AbsSeekBar {
    ...
    private void trackTouchEvent(MotionEvent event) {
        float scale;
        int th = this.mThumb != null ? this.mThumb.getIntrinsicHeight() : 0;
        int height = getHeight();
        int available = ((height - th) - this.mPaddingBottom) - this.mPaddingTop;
        ...
        if (eventY > height - getPaddingBottom()) {
            ...
            scale = ((float) (((height - this.mPaddingBottom) - eventY) - (th / 2))) / ((float) available);
        }
        setProgressOnMove((float) Math.round(((float) getMax()) * scale));
    }
